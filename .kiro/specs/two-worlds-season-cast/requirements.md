...
### R0 — Creative authorship model (models draft, owner approves, spine locked)
- R0.1 The models (DeepSeek R1/V3) do the heavy **creative** lifting — prose, dialogue,
  beats, character depth, episode situations, the proposed arc + Macal stage split.
- R0.2 The **owner** owns **brand + spine**: approves the regenerated series bible,
  supplies the non-negotiable brand guardrails as constraints, and locks the arc +
  stage split so all 10 episodes stay consistent.
- R0.3 **Brand guardrails (non-negotiable, fed to the models):** EEC brand voice +
  honesty rules; "learn with fun" mission; Macal-as-learner pedagogy (American target;
  L2→American arc); Dubai setting; cast realism; short 5-10 min format + structure rule.

### R1 — Season-level planning
- R1.0 The **series bible is regenerated by the models** (DeepSeek R1 as showrunner),
  within the R0.3 guardrails, and **owner-approved + locked** BEFORE the season plan and
  scripts. The existing bible is the starting reference, not the final word.
...
- R4.3 Every episode passes the existing **structure gate** (story/teaching separation,
  series-bible §10). Duration is INFORMATIONAL (the hard 5-10 min gate was removed per
  owner decision; ~7 min stays the target but is not enforced).
